<?php
/*
Plugin Name: TranslatePress - Personal
...
function trp_personal_update(){
    // this is the URL our updater / license checker pings. This should be the URL of the site with EDD installed
    define('TRP_PER_STORE_URL', 'https://translatepress.com');
    // the name of your product. This should match the download name in EDD exactly
    define('TRP_PER_ITEM_NAME', 'TranslatePress Personal');
    if (class_exists('TRP_EDD_SL_Plugin_Updater')) {
        // retrieve our license key from the DB
        $license_key = trim(get_option('trp_license_key'));
        // setup the updater
        $edd_updater = new TRP_EDD_SL_Plugin_Updater(TRP_PER_STORE_URL, __FILE__, array(
                'version' => '1.2.8',        // current version number
                'license' => $license_key,    // license key (used get_option above to retrieve from DB)
                'item_name' => TRP_PER_ITEM_NAME,    // name of this plugin
                'item_id' => '220',
...
add_action( 'plugins_loaded', 'trp_personal_update', 0 );
/** End the update initialization here **/


if( !defined( 'TRANSLATE_PRESS' ) )
    define( 'TRANSLATE_PRESS', 'TranslatePress - Personal' );


register_activation_hook(__FILE__, 'trp_personal_activate');
function trp_personal_activate( $network_wide ) {
    if( !function_exists('is_plugin_active') )
        include_once( ABSPATH . '/wp-admin/includes/plugin.php' );

    if( is_plugin_active('translatepress-business/index.php') || is_plugin_active('translatepress-developer/index.php') ){
        set_transient( 'trp_deactivate_personal', true );
    }else{
        delete_transient( 'trp_deactivate_personal' );
    }
}

add_action('admin_notices', 'trp_personal_admin_notice');
function trp_personal_admin_notice(){
    $trp_deactivate_personal = get_transient( 'trp_deactivate_personal' );
    if( $trp_deactivate_personal ){

        $other_plugin_name = '';
        if( is_plugin_active('translatepress-business/index.php') )
            $other_plugin_name = 'TranslatePress - Business';
...
add_action( 'admin_init', 'trp_personal_plugin_deactivate' );
function trp_personal_plugin_deactivate() {
    $trp_deactivate_personal = get_transient( 'trp_deactivate_personal' );
    if( $trp_deactivate_personal ){
